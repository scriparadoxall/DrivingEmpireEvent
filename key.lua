...
-- [CORREÇÃO 3] Melhoria de segurança: Tenta usar a CoreGui ou gethui() para esconder do Anti-Cheat
local CoreGui
if gethui then
    CoreGui = gethui()
else
...
local function StartMainScript(tier)
    ...
    if not success then warn("Erro ao carregar script: " .. tostring(err)) end
end

-- [Livraria de Criptografia - Mantida intacta]
...
-- [CORREÇÃO 4] Impedindo que a verificação de host trave a interface na injeção
task.spawn(function()
...
-- [CORREÇÃO 2] Evitando variável global (mudado para local)
...
local redeemPlatoKey = function(key)
    ...
    if response.StatusCode == 200 then
        ...
        if decodeOk and decoded.success == true then
            ...
            -- [CORREÇÃO 1] Evita erro de 'attempt to index a string value'
            local errMsg = ""
